...
#
# Description:
# zfs send disallows the source-side combination of -w and --redact at the
# CLI. The same nvlist combination (BEGINNV_REDACT_SNAPS together with
# crypt_keydata) can still be reached by:
...
# payload is XDR-encoded and the stream can be received.
#
# Strategy:
...
#    encryption).
# 4. zfs send -w of the received dataset to a second stream file.
# 5. verify_xdr_nvlist_encoding on the second stream.
# 6. zfs receive the second stream successfully.
#
...
log_assert "BEGIN nvlist of a raw send of a received-redacted dataset is XDR-encoded and receivable"
...
# Re-send raw. The DRR_BEGIN nvlist now carries both BEGINNV_REDACT_SNAPS
# (case 2, SPA_FEATURE_REDACTED_DATASETS active on to_ds) and crypt_keydata
...
log_pass "BEGIN nvlist of a raw send of a received-redacted dataset is XDR-encoded and receivable"
